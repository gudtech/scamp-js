...
util/config.js - configuration subsystem for scamp-js

=head1 SYNOPSYS

    scamp.config().scampAddr();
    scamp.config().cache_ping_interval;

=head1 DESCRIPTION

This module implements reading of the B</etc/scamp/main.conf> file, or an
...
var fs   = require('fs'),
    os   = require('os'),
    scamp = require('../index.js'),
    used = {},
    conf = '',
    path = process.env['SCAMP'] || '/etc/scamp/scamp.conf',
    values = {
    };
...
try {
    ...
    if (ex.code != 'ENOENT') {
        scamp.error('Failed to read config file!', aex);
    }
}
...
function probeInterface() {
    if (!probedAddr) {
        ...
        if (!probedAddr)
            scamp.fatal('No suitable address found in bus.address autoprobe');
    }
    return [probedAddr];
}
...
conf.split('\n').forEach(function (line) {
    ...
    var ix = line.indexOf('=');
    if (ix < 0) {
        scamp.error('Config line has no equals:', line);
        return;
    }
    ...
    if (used[variable]) {
        scamp.error('Duplicate config variable, using first instance:',variable);
        return;
    }
    used[variable] = true;
...
values.val = function(name, def) {
    if (this[name] === undefined && def === undefined) {
        scamp.fatal('Value required for', name);
    }
...
values.addressListVal = function (name) {
    ...
    this.val(name, '').split(',').map(function (raw) {
        ...
        if (addr)
            ...
            scamp.error("Cannot resolve",raw,"to an interface");
    });
