...
function Ticket() {
    this.requester = scamp.requester({ ident: 'scamp-js-ticket' });
...
Ticket.prototype.verify = function (ticket) {
    ...
    var sig = Buffer.from(parts.pop().replace(/-/g,'+').replace(/_/g,'/'), 'base64');

    pubkey = pubkey || fs.readFileSync('/etc/scamp/auth/ticket_verify_public_key.pem');
    if (!crypto.createVerify('sha256').update( Buffer.from(parts.join(',')) ).verify( pubkey, sig ))
        return null;
