...
class Connection extends EventEmitter {
    ...
    _ondata (buf) {
        ...
        if (buf.length + this._inEnd > this._inBuf.length) {
            ...
            let newBuf = Buffer.alloc( newSize < 128 ? 256 : newSize * 2 );
...
Connection.prototype._onpacket.HEADER = function(msgno, payload) {
    ...
    info.message.on('drain', function() {
        if (info.received != info.acked) {
            ...
            me._sendPacket('ACK', msgno, Buffer.from( String(info.acked) ));
        }
    });
};
...
Connection.prototype._onpacket.DATA = function (msgno, payload) {
    ...
    if (info.message.write(payload) !== false) {
        ...
        this._sendPacket('ACK', msgno, Buffer.from( String(info.acked) ));
    } else {
...
Connection.prototype.sendMessage = function(msg) {
    ...
    this._sendPacket('HEADER', id, Buffer.from(JSON.stringify(msg.header)));
    ...
    msg.on('end', function () {
        ...
        if (msg.error) {
            me._sendPacket('TXERR', id, Buffer.from(msg.error));
        } else {
            me._sendPacket('EOF', id, Buffer.from(''));
        }
    });
};
