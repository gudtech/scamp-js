'use strict';
// scamp-js/lib/handle/service.js
...
function Service( text, fingerprint ) {
    ...
    if (typeof ref.uri !== 'string') throw 'address is required';

    me.workerIdent = fingerprint + '#' + ref.ident;
    me.address = ref.uri;
    require('url').parse(ref.uri); // just in case they get any funny ideas
    me.weight = ref.wgt;
...
var authorized_keys_name = scamp.config().val('bus.authorized_services', process.env.SCAMP_CONFIG_DIR + '/authorized_services');
var fs = require('fs');
...
Service.prototype.authorized = function (sector, action) {
    ...
    if (!rx) {
        //if (!this.unauth_warn) scamp.error('Unauthorized service',this.fingerprint);
        this.unauth_warn = {};
        return false;
    }
    ...
    if (!this.unauth_warn[action]) {
        ...
        //scamp.error('Service',this.fingerprint,'not authorized to provide',action);
    }
    return false;
};
