...
// Pinboard server process: listens on port 51358/SCAMPv1 and allows SCAMP services to announce their presence
...
var tls          = require('tls'),
    scamp        = require('../lib/index.js'),
    conn         = scamp.module('transport/beepish/connection.js'),
...
function InConnection(clear, statemgr) {
    ...
    scamp.debug('New inbound connection');
...
function ConnectionServer(params) {
    ...
    me._tlssrv.listen( params.port, params.address, function () {
        scamp.info("Listening for connections on", params.port, params.address);
    } );
}
